...
- 所有的线程共享Java堆，在这里还可以**划分线程私有的缓冲区**，可以看为线程安全的（Thread Local Allocation Buffer，TLAB）。
...
Exception in thread "main" java.lang.OutOfMemoryError: Java heap space
	at code.JVM.chapter8.Picture.<init>(OOMtest.java:24)
	at code.JVM.chapter8.OOMtest.main(OOMtest.java:15)
```
...
![java堆区划分](../base/java堆区划分.png)

### 配置
**下面这些参数开发中一般不会调：
![年轻代与老年代空间分配](../base/年轻代与老年代空间分配.png)
- 配置新生代与老年代在堆结构的占比
...
6. 啥时候能去养老区呢？可以设置次数。默认是15次。
   - 可以设置新生区进入养老区的年龄限制，**可以设置 JVM 参数：-XX:MaxTenuringThreshold=<N> 进行设置**
...
![对象分配与回收过程](../base/对象分配与回收过程.png)
![对象分配与回收过程_1](../base/对象分配与回收过程_1.png)
![对象分配与回收过程_2](../base/对象分配与回收过程_2.png)

### 总结
- 针对幸存者s0，s1区的总结：复制之后有交换，谁空谁是to
- 关于垃圾回收：频繁在新生区收集，很少在养老区收集，几乎不在永久区/元空间收集。
### 图解对象分配（特殊情况）
![对象分配与回收过程_特殊情况](../base/对象分配与回收过程_特殊情况.png)
...
1. VM在进行GC时，并非每次都对上面三个内存区域一起回收的，大部分时候回收的都是指新生代。
2. 针对Hotspot VM的实现，它里面的GC按照回收区域又分为两大种类型：一种是部分收集（Partial GC），一种是整堆收集（FullGC）
...
**最简单的分代十GC策略的触发条件**
![最简单的分代式GC策略](../base/最简单的分代式GC策略.png)
...
- 在程序中，开发人员可以通过选项**-XX:UseTLAB**设置是否开启TLAB空间，默认TLAB是开启的。
- 默认情况下，TLAB空间的内存非常小，仅占有整个Eden空间的1%，当然我们可以通过选项**-XX:TLABWasteTargetPercent**设置TLAB空间所占用Eden空间的百分比大小。
- 一旦对象在TLAB空间分配内存失败时，**JVM就会尝试着通过使用加锁机制确保数据操作的原子性**，从而直接在Eden空间中分配内存。
![对象分配_TLAB](../base/对象分配_TLAB.png)
...
- -XX:+PrintGCDetails：输出详细的GC处理日志。下面两个打印的简单的GC处理日志，效果相同
  - -XX:+PrintGC
  - -verbose:gc
...
- 如果小于，则虚拟机会查看**-XX:HandlePromotionFailure**设置值是否允担保失败。
  - 如果HandlePromotionFailure=true，那么会继续检查**老年代最大可用连续空间是否大于历次晋升到老年代的对象的平均大小**。
    ...
    - 如果小于，则进行一次Full GC。
- 在JDK6 Update 24之后（JDK7及之后），**HandlePromotionFailure参数不会再影响到虚拟机的空间分配担保策略，即只会是true**，观察openJDK中的源码变化，虽然源码中还定义了HandlePromotionFailure参数，但是在代码中已经不会再使用它。JDK6 Update 24之后的规则变为只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小就会进行Minor GC，否则将进行Full GC。即 HandlePromotionFailure=true

## 堆是分配对象的唯一选择吗
**在《深入Java虚拟机》中关于Java堆内存有这样一段描述；
...
**逃逸分析举例**
1. 有发生逃逸的对象，则可以分配到栈（无线程安全问题）上，随着方法执行的结束，栈空间就被移除（也就无需GC）
```
...
3. 标量替换为栈上分配提供了很好的基础。
4. 参数 -XX:+ElimilnateAllocations：开启了标量替换（默认打开），允许将对象打散分配在栈上。
```
/**
 * 标量替换测试
...
```
上述代码在主函数中进行了一亿次alloc。调用进行对象创建，由于User对象实例需要占据约16字节的空间，因此累计分配空间达到将近1.5GB。如果堆空间小于这个值，就必然会繁盛GC。使用如下参数运行上述代码：
